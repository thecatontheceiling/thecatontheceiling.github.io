...
(function () {
    ...
    const settings = {"cursor_width":16,"cursor_height":20,"cursor_opacity":10,"cursor_fill":"rgb(100, 76, 110)","cursor_outline":"rgb(255, 255, 255)","left_box":"","right_box":""}
    const iid = "1578"
    ...
    const ws = resilientWebSocket(`wss://widget.menal.xyz/cursors/ws?iid=${encodeURIComponent(iid)}&path=${encodeURIComponent(location.pathname)}`)
    ...
    let scrollTimer = 0
    function sendScrollEndPos() {
        scrollTimer = 0
        if (!hasMouse) return
        lastSend = performance.now()
        sendPos(Math.round(lastClientX + window.scrollX), Math.round(lastClientY + window.scrollY))
    }
    window.addEventListener("scroll", () => {
        if (!hasMouse) return
        if (scrollTimer) clearTimeout(scrollTimer)
        scrollTimer = setTimeout(sendScrollEndPos, 150)
    }, { passive: true })
